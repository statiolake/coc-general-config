...
var DemoList = class extends import_coc.BasicList {
  constructor() {
    super();
    this.name = "demo_list";
    this.description = "CocList for coc-general-config";
  ...
  async loadItems(context) {
    return [
      {
        label: "coc-general-config list item 1",
        data: { name: "list item 1" }
      },
      {
        label: "coc-general-config list item 2",
...
async function activate(context) {
  import_coc2.window.showInformationMessage("coc-general-config works!");
  context.subscriptions.push(
    import_coc2.commands.registerCommand("coc-general-config.Command", async () => {
      import_coc2.window.showInformationMessage("coc-general-config Commands works!");
    }),
    import_coc2.listManager.registerList(new DemoList()),
    import_coc2.sources.createSource({
      name: "coc-general-config completion source",
      // unique id
      doComplete: async () => {
